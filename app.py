...
# === Config del negocio (desde config_precios.py) ===
WHATSAPP_NUMBER = os.getenv("LAVA_WHATSAPP", cfg.WEB_CONFIG["whatsapp"])
LAVA_DIRECCION = os.getenv("LAVA_DIRECCION", cfg.WEB_CONFIG["direccion"])
PROMO_BANNER = os.getenv("LAVA_PROMO", "")
...
def inject_globals():
    ...
    promocion_hoy = cfg.get_promocion_dia()
    promo_texto = promocion_hoy["descripcion"] if promocion_hoy else PROMO_BANNER
    
    return dict(
        WHATSAPP_NUMBER=WHATSAPP_NUMBER,
        LAVA_DIRECCION=LAVA_DIRECCION,
        PROMO_BANNER=promo_texto,
        now=datetime.now(),
        title="Lavandería RÍOS",
        meta_description=cfg.WEB_CONFIG["meta_description"],
        meta_keywords=cfg.WEB_CONFIG["keywords"]
    )
...
def home():
    return render_template("index.html", 
                          title="Inicio",
                          og_title="Servicio profesional de lavandería en Huánuco")
    ...
    if request.method == "POST":
        try:
            ...
            if not cliente:
                ...
                return render_template("boleta_nueva.html", title="Nueva Boleta")
...
def boleta_detalle(boleta_id):
    ...
    return render_template("boleta_detalle.html", cab=cab, items=items, wa_link=wa_link, 
                          title=f"Boleta #{boleta_id} - {cab[2] if cab else ''}")

# Ruta para el Service Worker (PWA)
@app.route('/sw.js')
def service_worker():
    return app.send_static_file('sw.js')

# Ruta para el manifest.json (PWA)
